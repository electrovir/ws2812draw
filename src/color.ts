/**
 * Color examples. The format is WWBBGGRR. WW is unused. These directly affect the brightness of
 * each colored LED. Thus, if you turn these values way up, the lights will be, in general,
 * brighter. There's a direct impact on how effective the brightness parameter is. For example, low
 * color values here might not even show up if the brightness is too low. These colors are picked so
 * that they'll mostly even out at higher brightnesses but they may not be unique in lower
 * brightness. For example, at brightness 7 ORANGE and RED look the same.
 */
